  ...
  if (function == ADnEDResetParam) {
    ...
    if (value) {
      if (adStatus == ADStatusAcquire) {
        ...
        cout << "ADStatusAcquire::" <<ADStatusAcquire <<endl; 
        ...
        //Note: this does not work. I need to clear the busy from outside this function.
        setIntegerParam(ADnEDStopParam, 1);
        callParamCallbacks();
        return asynError;
      }
    }
...
void ADnED::eventHandler(shared_ptr<epics::pvData::PVStructure> const &pv_struct, epicsUInt32 channelID)
{
  ...
  epics::pvData::PVULongPtr pT0HighPtr = pv_struct->getSubField<epics::pvData::PVULong>(ADNED_PV_T0HIGH);
  epics::pvData::PVULongPtr pT0LowPtr = pv_struct->getSubField<epics::pvData::PVULong>(ADNED_PV_T0LOW);
  uint64_t mT0High = pT0HighPtr->get();
  uint64_t mT0Low = pT0LowPtr->get();
  //uint64_t  mT0High = 0x000000FFFFFFFFFF&mT0High;
  //mT0Low = ;
  //cout<<"the T0 high is ::" <<mT0High<<endl;
  ...
  if (pixelsPtr && tofPtr) {
    ...
    m_eventPulseTime[0] = mT0High;
    //new stucture para


    uint32_t *everyPulsePixelData;
    everyPulsePixelData = new uint32_t[pixelsLength];
    uint32_t *everyPulseTOFData;
    everyPulseTOFData = new uint32_t[pixelsLength];
    uint64_t everyPulseEventNum[1];
        ...
        switch(hdfFileWriteMode){
              ...
              case 1:
                ...
                char *file_time; 
                  ...
                  char mFileName_day[2]={'0'};
                  char mFileName_hour[2]={'0'};
                  char mFileName_min[2]={'0'};
                  char mFileName_second[2]={'0'};
                  //char single_file[256]={'0'};
                  int flag =0;
                  const char *sep = ":, ";
                  p = strtok(file_time, sep); 
                  while(p){
                
                  switch(flag){
                    case 1:
                      strcpy(mFileName_month,p);
                      cout<<" month ="<<mFileName_month<<endl;
                      switch(mFileName_month[2]){
                        case 'n':
                          if(mFileName_month[0]=='F')
                            {mFileName_month[0]='0';mFileName_month[1]='1';break;}
                          if(mFileName_month[0]=='J')
                            {mFileName_month[0]='0';mFileName_month[1]='6';break;}
                        case 'b':
                          mFileName_month[0]='0';mFileName_month[1]='2';break;
                        case 'r':
                          if(mFileName_month[0]=='M')
                            {mFileName_month[0]='0';mFileName_month[1]='3';break;}
                          if(mFileName_month[0]=='A')
                            {mFileName_month[0]='0';mFileName_month[1]='4';break;}
                        case 'y':
                          mFileName_month[0]='0';mFileName_month[1]='5';break;
                        case 'l':
                          mFileName_month[0]='0';mFileName_month[1]='7';break;
                        case 'g':
                          mFileName_month[0]='0';mFileName_month[1]='8';mFileName_month[2] = '\0';
                          cout<<"aaaa=="<<mFileName_month<<endl;
                          break;
                        case 'p':
                          mFileName_month[0]='0';mFileName_month[1]='9';break;
                        case 't':
                          mFileName_month[0]='1';mFileName_month[1]='0';break;
                        case 'v':
                          mFileName_month[0]='1';mFileName_month[1]='1';break;
                        case 'c':
                          mFileName_month[0]='1';mFileName_month[1]='2';break;

                      }
                      
                        break;
                    case 2:
                      strcpy(mFileName_day,p);
                      mFileName_day[1]=mFileName_day[0];
                      mFileName_day[0] = '0';
                      break;
                    case 3:
                      strcpy(mFileName_hour,p);
                      break;
                    case 4:
                      strcpy(mFileName_min,p);
                      break;
                    case 5:
                      strcpy(mFileName_second,p);
                      break;
                    case 6:
                      memcpy(mFileName_year,p,strlen(p)-1);
                      break;
                ...
                NXmakegroup (capture_file_id, "csns", "NXentry")  ;
                NXopengroup (capture_file_id, "csns", "NXentry")  ;   

                
                capture_file_num++;

                NXmakegroup (capture_file_id, "instrument", "NXdata");
                NXopengroup (capture_file_id, "instrument", "NXdata");

                NXmakegroup (capture_file_id, "monitor0", "NXdata");
                NXopengroup (capture_file_id, "monitor0", "NXdata");
                
								NXcompmakedata64 (capture_file_id, "event_pixel_id", NX_INT32, 1, unlimited_dims,NX_COMP_LZW,&dim);
                NXopendata (capture_file_id, "event_pixel_id");
                NXclosedata (capture_file_id);

                NXcompmakedata64 (capture_file_id, "event_time_of_flight", NX_UINT32, 1, unlimited_dims,NX_COMP_LZW,&dim);
                NXopendata (capture_file_id, "event_time_of_flight");
                NXclosedata (capture_file_id);
              
                NXcompmakedata64 (capture_file_id, "event_pulse_time", NX_UINT64, 1, unlimited_dims,NX_COMP_LZW,&dim1);
                ...
                start_every_slap = 0;
                    ...
                    //cout<<"the start slab::"<<slab_start[0]<<endl; 
                    NXputslab64 (capture_file_id, everyPulsePixelData, slab_start,slab_size);
                    NXclosedata (capture_file_id);
                    ...
                    NXopenpath (capture_file_id,"/csns/instrument/monitor0");
                    NXopendata (capture_file_id, "event_time_of_flight");
                    NXputslab64 (capture_file_id, everyPulseTOFData, slab_start,slab_size);
                    ...
                    NXopenpath (capture_file_id,"/csns/instrument/monitor0");
                    NXopendata (capture_file_id, "event_index");
                    NXputslab64 (capture_file_id, m_eventIndex, slab_start,slab_size);
                    NXclosedata (capture_file_id);
                    NXflush (&capture_file_id);
              ...
              case 2:
                ...
                { 
                  ...
                  if (nxstat == NX_ERROR) {
                   ...
                   // NXcompmakedata (stream_file_id, "GEM_HV_1", NX_CHAR, 1, &common_aChar_len,NX_COMP_LZW,&dim);
                   // NXopendata (stream_file_id, "GEM_HV_1");
                   // NXputdata (stream_file_id, hdfHV1Message);
                   // NXclosedata (stream_file_id);

                   // NXcompmakedata (stream_file_id, "GEM_HV_2", NX_CHAR, 1, &common_aChar_len,NX_COMP_LZW,&dim);
                   // NXopendata (stream_file_id, "GEM_HV_2");
                   // NXputdata (stream_file_id, hdfHV2Message);
                   // NXclosedata (stream_file_id);

                   // NXcompmakedata (stream_file_id, "GEM_gas", NX_CHAR, 1, &common_aChar_len,NX_COMP_LZW,&dim);
                   // NXopendata (stream_file_id, "GEM_gas");
                   // NXputdata (stream_file_id, hdfGasContentMessage);
                   // NXclosedata (stream_file_id);

                    NXmakegroup (stream_file_id, "NeutronData", "NXdata");
                    NXopengroup (stream_file_id, "NeutronData", "NXdata");

                    
                    //NXmakedata(stream_file_id, "event_pixel_id", NX_UINT32, 1, unlimited_dims);
                    NXcompmakedata64 (stream_file_id, "event_pixel_id", NX_UINT32, 1, unlimited_dims,NX_COMP_LZW,&dim);
                    NXopendata(stream_file_id, "event_pixel_id");
                    NXclosedata(stream_file_id);
                  //  NXflush (&stream_file_id);

                    NXcompmakedata64 (stream_file_id, "event_time_of_flight", NX_UINT32, 1, unlimited_dims,NX_COMP_LZW,&dim);
                   // NXmakedata(stream_file_id, "event_time_of_flight", NX_UINT32, 1, unlimited_dims);
                    NXopendata(stream_file_id, "event_time_of_flight");
                    NXclosedata (stream_file_id);

                  NXcompmakedata64 (stream_file_id, "event_num_per_pulse", NX_UINT64, 1, unlimited_dims,NX_COMP_LZW,&dim1);
                ...
                /*Output the pxiel ID */
                NXopenpath(stream_file_id,"/entry/NeutronData");
                NXopendata(stream_file_id, "event_pixel_id");
                slab_start[0] = start_every_slap;slab_size[0] = pixelsLength;
                //cout<<"the start slab::"<<slab_start[0]<<endl; 
                NXputslab64(stream_file_id, everyPulsePixelData, slab_start,slab_size);
                NXclosedata(stream_file_id);
                NXflush(&stream_file_id);
              
                NXopenpath (stream_file_id,"/entry/NeutronData");
                NXopendata (stream_file_id, "event_time_of_flight");   
                NXputslab64 (stream_file_id, everyPulseTOFData, slab_start,slab_size);
                ...
                NXopenpath (stream_file_id,"/entry/NeutronData");
                NXopendata (stream_file_id, "event_num_per_pulse");
                slab_start[0] = stream_group_num*2; slab_size[0] = 2;
               // cout << slab_start[0] << "      "  << slab_size[0] << endl;
                NXputslab64(stream_file_id, m_eventPulseTime, slab_start,slab_size);
                NXclosedata (stream_file_id);
                NXflush (&stream_file_id);

                NXclosegroup (stream_file_id);   
                start_every_slap += static_cast<int64_t>(pixelsLength);
                stream_group_num++ ;
                 break;
              
            }
            
             m_eventIndex[0] +=pixelsLength;
             delete [] everyPulsePixelData;
             delete [] everyPulseTOFData;
  ...
  while (1) {
    ...
    while (acquire) {
      ...
      if (eventStatus == epicsEventWaitOK) {
        ...
        cout<< "the flag ::" <<stream_group_num<<endl; 
        stream_group_num = 0;
